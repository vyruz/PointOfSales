...
class App:
	...
	def orderWindow(self, order):
		...
		E3 = Button(root, text = shroomBurger.getName(), command = lambda: self.addComment(order, shroomBurger, comBox, listbox)).grid(row = 4, column = 0)
		E4 = Button(root, text = baconBurger.getName(), command = lambda: self.addComment(order, baconBurger, comBox, listbox)).grid(row = 5, column = 0)

		A1 = Button(root, text = fries.getName(), command = lambda: self.addComment(order, fries, comBox, listbox)).grid(row = 2, column = 1)
		A2 = Button(root, text = onionRings.getName(), command = lambda: self.addComment(order, onionRings, comBox, listbox)).grid(row = 3, column = 1)
		A3 = Button(root, text = chips.getName(), command = lambda: self.addComment(order, chips, comBox, listbox)).grid(row = 4, column = 1)
		A4 = Button(root, text = chili.getName(), command = lambda: self.addComment(order, chili, comBox, listbox)).grid(row = 5, column = 1)

		S1 = Button(root, text = coke.getName(), command = lambda: self.addComment(order, coke, comBox, listbox)).grid(row = 2, column = 2)
		S2 = Button(root, text = drPepper.getName(), command = lambda: self.addComment(order, drPepper, comBox, listbox)).grid(row = 3, column = 2)
		S3 = Button(root, text = sprite.getName(), command = lambda: self.addComment(order, sprite, comBox, listbox)).grid(row = 4, column = 2)
		S4 = Button(root, text = rootBeer.getName(), command = lambda: self.addComment(order, rootBeer, comBox, listbox)).grid(row = 5, column = 2)
		S5 = Button(root, text = lemonade.getName(), command = lambda: self.addComment(order, lemonade, comBox, listbox)).grid(row = 6, column = 2)

		H1 = Button(root, text = beerShake.getName(), command = lambda: self.addComment(order, beerShake, comBox, listbox)).grid(row = 2, column = 3)
